...
public class MultiPopCoevolutionaryEvaluatorMO extends Evaluator
    {
    ...
    public void evaluatePopulation(final EvolutionState state)
        ...
        ((GroupedProblemForm)p_problem).preprocessPopulation(state, state.population, null, false);
        performCoevolutionaryEvaluation( state, state.population, (GroupedProblemForm)p_problem );
        ((GroupedProblemForm)p_problem).postprocessPopulation(state, state.population, null, false);
    ...
    public void performCoevolutionaryEvaluation( final EvolutionState state,
        ...
        for( int i = 0 ; i < selectionMethodPrev.length ; i++ )
            {
            if (numCurrent>0) selectionMethodCurrent[i].prepareToProduce( state, i, 0 );

            // do a hack here
            Population currentPopulation = state.population;
            state.population = previousPopulation;
            if (numPrev>0) selectionMethodPrev[i].prepareToProduce( state, i, 0 );
            ...
            for(int i = 0; i < state.population.subpops[j].individuals.length; i++)
                ...
                for(int k = 0; k < numCurrent; k++)
                    ...
                    for(int ind = 0; ind < inds.length; ind++)
                        {
                        if (ind == j) { inds[ind] = state.population.subpops[j].individuals[i]; updates[ind] = true; }
                        else { inds[ind] = produce(selectionMethodCurrent[j], ind, i, state, 0); updates[ind] = false; }
                        }
                    prob.evaluate(state,inds,updates, false, subpops, 0);
                    }
                ...
                for(int k = 0; k < numPrev; k++)
                    ...
                    for(int ind = 0; ind < inds.length; ind++)
                        ...
                        if (ind == j) { inds[ind] = state.population.subpops[j].individuals[i]; updates[ind] = true; }
                            ...
                            Population currentPopulation = state.population;
                            state.population = previousPopulation;
                            inds[ind] = produce(selectionMethodPrev[j], ind, i, state, 0);
                            state.population = currentPopulation;
                            ...
                            {
                            inds[ind] = state.population.subpops[ind].individuals[
                                state.random[0].nextInt(state.population.subpops[ind].individuals.length)];
                            updates[ind] = false;
                            }
                        }
                    prob.evaluate(state,inds,updates, false, subpops, 0);
        ...
        for( int i = 0 ; i < selectionMethodPrev.length ; i++ )
            {
            if (numCurrent>0) selectionMethodCurrent[i].finishProducing( state, i, 0 );

            // do a hack here
            Population currentPopulation = state.population;
            state.population = previousPopulation;
            if (numPrev>0) selectionMethodPrev[i].finishProducing( state, i, 0 );
            state.population = currentPopulation;
            }
        }
    ...
    void loadElites( final EvolutionState state,
        ...
        if (numElite==1)
            ...
            QuickSort.qsort(orderedPop, new EliteComparatorNew(subpop.individuals));
...
class EliteComparatorNew implements SortComparatorL
    {
    Individual[] inds;
    public EliteComparatorNew(Individual[] inds) {super(); this.inds = inds;}
