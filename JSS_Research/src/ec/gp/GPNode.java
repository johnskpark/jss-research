...
public abstract class GPNode implements GPNodeParent, Prototype
    ...
    */
    ...
    public int printNodeForHumans(final PrintStream ps)
    	{
        String n = toStringForHumans();
        ps.print(n);
        return n.length();
    ...
    */
    ...
    public int printRootedTreeForHumans(final PrintStream ps,
    	int tablevel, int printbytes)
    	{
        if (printbytes>MAXPRINTBYTES)
        {
        ...
        printbytes = 0;
        for(int x=0;x<tablevel;x++)
            ps.print(GPNODEPRINTTAB);
        }

	    if (children.length>0) { ps.print(" ("); printbytes += 2; }
	    else { ps.print(" "); printbytes += 1; }

	    printbytes += printNodeForHumans(ps);

	    for (int x=0;x<children.length;x++)
	        printbytes = children[x].printRootedTreeForHumans(ps,tablevel,printbytes);
	    if (children.length>0) { ps.print(")"); printbytes += 1; }
	    return printbytes;
    	}
