...
public class NSGA2EvaluatorCoevolve_backup extends MultiPopCoevolutionaryEvaluatorMO
    {
    ...
    public ArrayList archive = new ArrayList();
    ...
    public Individual[] buildArchive(EvolutionState state, int subpop)
        ...
        assignSparsity(state.population.subpops[subpop].individuals);
        ArrayList nonOverlapPop = new ArrayList();
            //System.out.println("**************************");
        for (int i = 0; i < state.population.subpops[subpop].individuals.length; i++) {
            ...
            if (((NSGA2MultiObjectiveFitness) ind.fitness).sparsity!=0&&((MultiObjectiveFitness) ind.fitness).getObjective(0)!=1000000) {
                nonOverlapPop.add(ind);
                //System.out.println(((MultiObjectiveFitness) ind.fitness).getObjective(0) + " " +((MultiObjectiveFitness) ind.fitness).getObjective(1) + " " +((MultiObjectiveFitness) ind.fitness).getObjective(2));
            }
        }
        Individual[] nonLapInds = (Individual[])(nonOverlapPop).toArray(dummy);
               
        ArrayList ranks = assignFrontRanks(nonLapInds);
        ...
        for(int i = 0; i < size; i++)
            ...
            if (newSubpopulation.size() < originalPopSize[subpop])
            {
                int left = originalPopSize[subpop] - newSubpopulation.size();
                for(int j = 0 ; j < left; j++){
                    Individual ind = state.population.subpops[subpop].species.newIndividual(state, 0);
                    ((NSGA2MultiObjectiveFitness) ind.fitness).rank = size; 
                    newSubpopulation.add(ind);
                }
                System.out.println("new random:" + left);
            } else System.out.println("rank 0:" + ((ArrayList)(ranks.get(0))).size());
        

        Individual[] archive = (Individual[])(newSubpopulation.toArray(dummy));
                
        // maybe force reevaluation
        NSGA2BreederCoevolve breeder = (NSGA2BreederCoevolve)(state.breeder);
