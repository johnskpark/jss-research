...
public class CrossoverPipelineNew extends GPBreedingPipeline
    {
    ...
    public CrossoverPipelineNew() { parents = new GPIndividual[2]; }
    ...
    public Object clone()
        {
        CrossoverPipelineNew c = (CrossoverPipelineNew)(super.clone());
    ...
    public int produce(final int min, 
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
            ...
            parents[1] = (GPIndividual) BinarySelection(state, thread, subpopulation, state.population.archive) ;
        ...
        public Individual BinarySelection(final EvolutionState state, final int thread, final int subpop,final Individual[] inds){
            int ind1 = state.random[thread].nextInt(inds.length);
            int ind2 = state.random[thread].nextInt(inds.length);
            if (((NSGA2MultiObjectiveFitness)(inds[ind1].fitness)).sparsity > ((NSGA2MultiObjectiveFitness)(inds[ind2].fitness)).sparsity){
                {
                    return swapCloneContext((Coevolutionary2WayGPIndividual)inds[ind1],subpop);
                }
            } else 
                return swapCloneContext((Coevolutionary2WayGPIndividual)inds[ind2],subpop);
        }
