...
/**
 * MutateERFPipeline works very similarly to the "Gaussian" algorithm
 * described in Kumar Chellapilla,
 * "A Preliminary Investigation into Evolving Modular Programs without Subtree
 * Crossover", GP98.
 *
 * <p>MutateERFPipeline picks a random node from a random tree in the individual,
 * using its node selector.  It then proceeds to "mutate" every ERF (ephemeral
 * random function) located in the subtree rooted at that node.  It does this
 * by calling each ERF's <tt>mutateERF()</tt> method.  The default form of <tt>mutateERF()</tt>
 * method is to simply call <tt>resetNode()</tt>, thus randomizing the ERF;
...
public class MutateERFPipeline extends GPBreedingPipeline
    {
    public static final String P_MUTATEERF = "mutate-erf";
    ...
    public Parameter defaultBase() { return GPBreedDefaults.base().push(P_MUTATEERF); }

    public int numSources() { return NUM_SOURCES; }

    public Object clone()
        {
        MutateERFPipeline c = (MutateERFPipeline)(super.clone());
    ...
    public final void mutateERFs(final GPNode node, 
        final EvolutionState state, final int thread)
        {
        // is node an erf?
        if (node instanceof ERF)
            ((ERF)node).mutateERF(state,thread);

        // mutate children
        for(int x=0;x<node.children.length;x++)
            mutateERFs(node.children[x],state,thread);
        }
    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            if (tree!=TREE_UNFIXED && (tree<0 || tree >= i.trees.length))
                // uh oh
                state.output.fatal("MutateERFPipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual"); 
            ...
            // mutate all the ERFs in p1's subtree

            mutateERFs(p,state,thread);
