...
public class GPjspDDAHybrid extends GPProblem implements SimpleProblemForm {
    ...
    public Object clone(){
        GPjspDDAHybrid newobj = (GPjspDDAHybrid) (super.clone());
                ...
                for (int ds = 0; ds < 1; ds++) {
                    ...
                    while (!jspDynamic.isStop()) {
                        if (jspDynamic.isNextArrivalEvent()) {
                            ...
                            input.stat.tempDuate = input.tempVal;
                            if (input.stat.tempDuate < 0.0 || input.stat.tempDuate
                                    == Double.POSITIVE_INFINITY || Double.isNaN(input.stat.tempDuate)){
                            ...
                            for (int i = 0; i < newjob.getNumberOperations(); i++) {
                                ...
                                //input.partialEstimatedFlowtime += input.stat.OT + input.stat.QWL + input.stat.LOT ;
                            }
                    ...
                    while (!jspDynamic.isStop()) {
                        if (jspDynamic.isNextArrivalEvent()) {
                            ...
                            for (int i = 0; i < newjob.getNumberOperations(); i++) {
                                ...
                                input.tempVal = 0;
                                input.k = i;
                                ((GPIndividual)best_i).trees[1].child.eval(
                                    state,threadnum,input,stack,((GPIndividual)best_i),this);
                                input.partialEstimatedFlowtime += input.tempVal;
                                //input.partialEstimatedFlowtime += input.stat.OT + input.stat.QWL + input.stat.LOT ;
                            }
