...
public class GPjsp2WayMOCoevolveSPEA extends GPProblem implements GroupedProblemForm {
    ...
    public Object clone(){
        GPjsp2WayMOCoevolveSPEA newobj = (GPjsp2WayMOCoevolveSPEA) (super.clone());
    ...
    {
        boolean eval = false;
        if (updateFitness[0]&&!ind[0].evaluated) eval = true;
        if (updateFitness[1]&&!ind[1].evaluated) eval = true;
        if (eval){
            if( ind.length != 2 ||
                    ...
                    while (!jspDynamic.isStop()) {
                        if (jspDynamic.isNextArrivalEvent()) {
                            ...
                            for (int i = 0; i < newjob.getNumberOperations(); i++) {
                                ...
                                input.tempVal = 0;
                                input.k = i;
                                ((GPIndividual)ind2).trees[0].child.eval(
                                    state,threadnum,input,stack,((GPIndividual)ind2),this);
                            ...
                            do {
                                ...
                                if (M.getQueue().size()>1){
                                    ((GPIndividual)ind1).trees[0].child.eval(
                                        state,threadnum,input,stack,((GPIndividual)ind1),this);
        ...
        public String getTestPerformance(final EvolutionState state, final int threadnum, Individual[] ind){
            if( ind.length != 2 ||
                ( ! ( ind[0] instanceof Coevolutionary2WayGPIndividual ) ) ||
                ( ! ( ind[1] instanceof Coevolutionary2WayGPIndividual ) ) )
                {
    ...
    public void postprocessPopulation(final EvolutionState state, Population pop, boolean[] prepareForFitnessAssessment, boolean countVictoriesOnly)
    {
    	...
    	if (state.generation == state.numGenerations-1) {
    		...
    		myMOStat.myFinalStatisticCoevolveSPEA(state, 0,this);
