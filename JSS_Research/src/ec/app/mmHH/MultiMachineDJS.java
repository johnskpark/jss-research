...
public class MultiMachineDJS extends GPProblem implements SimpleProblemForm
{
	private static final long serialVersionUID = 1;

	public final static String P_MACHINES = "machines";
	public final static String P_OPERATIONS = "operations";
	public final static String P_CHANGESEED = "changeseed";
	public final static String P_NUMTREE= "numtree";
	public final static String P_DATA = "data";
	public final static String P_WARMUP = "warmup";
	public final static String P_JOBMEAS = "jobmeas";
	public final static String P_QUEUESIZE = "queuesize";
	public final static String P_VERBSIM = "verbsim";
	public final static String P_NUMMACHEVOL = "nummachevol";
	public final static String P_SEED = "seed";
	public final static String P_NMEASPROP = "nmeasprop";

	public ShopJob currentJob;
	public ShopMachine currentMachine;

	public int numMachines;
	public int numOps;

	public int numOpsConstant=1;

	public int warmup; // warm up period for dynamic job shop
	public double nmeasureProp;
	public int nmeasureTRAIN; // number of jobs we want to calculate objective function from during evolution
	public int nmeasure; // number of jobs to be used in testing
	public int size;   // number of jobs we want to create
	public int Qsize; // queue capacity at each machine

	public int numProcessed; // Number of jobs completely processed.

	public ShopJob[] jobs;
	public ShopMachine[] machines;


	public int VERB; // 1 => simulation details printed out, 0 => no details
	public static final double MAX = Double.MAX_VALUE;

	public double currentTime;

	public int changeseed;
	public int numTree;
	...
	public static enum DIST {UNIFORM, NEGBIN,EXPONENTIAL,GEOMETRIC};

	public int numMachEvol; // number of machine which evolved rules are being used to evaluate.

	public void setup(final EvolutionState state, final Parameter base)
	{
		// very important according to ECJ.
		super.setup(state,base);

		//Verifty input is of the right class
		...
		//attempt here to load simulation parameters?!
		Parameter p = base.push(P_MACHINES);
		numMachines = (int)(state.parameters.getIntWithDefault(p,def.push(P_MACHINES),10));

		p = base.push(P_OPERATIONS);
		numOps = (int)(state.parameters.getIntWithDefault(p,def.push(P_OPERATIONS),10));
		//		state.output.systemMessage(" "+changeseed);

		p = base.push(P_CHANGESEED);
		changeseed = (int)(state.parameters.getIntWithDefault(p,def.push(P_CHANGESEED),10));
		//		state.output.systemMessage(" "+changeseed);

		p = base.push(P_NUMTREE);
		numTree = (int)(state.parameters.getIntWithDefault(p,def.push(P_NUMTREE),1));
		//		state.output.systemMessage(" "+numTree);

		p= base.push(P_WARMUP);
		warmup = (int)(state.parameters.getIntWithDefault(p, def.push(P_WARMUP), 1000));

		p= base.push(P_JOBMEAS);
		nmeasure = (int)(state.parameters.getIntWithDefault(p, def.push(P_JOBMEAS), 5000));

		p= base.push(P_NMEASPROP);
		nmeasureProp = (int)(state.parameters.getIntWithDefault(p, def.push(P_NMEASPROP), 1));
		nmeasureTRAIN = (int)(nmeasure*nmeasureProp);

		p= base.push(P_QUEUESIZE);
		size = (int)(state.parameters.getIntWithDefault(p, def.push(P_QUEUESIZE), 50000));

		p= base.push(P_VERBSIM);
		VERB = (int)(state.parameters.getIntWithDefault(p, def.push(P_VERBSIM), 0));

		p= base.push(P_NUMMACHEVOL);
		numMachEvol = (int)(state.parameters.getIntWithDefault(p, def.push(P_NUMMACHEVOL), 10));

		p= base.push(P_SEED);
		seed = (int)(state.parameters.getIntWithDefault(p, def.push(P_SEED), 10));
	}
		...
		if(!ind.evaluated) // don't bother reevaluating
		{
			...
			for(int i=0;i<seeds.length;i++){
				if(changeseed==10){
					...
					value = value + simulate(state, ind, subpopulation, threadnum, util[i], DIST.UNIFORM, mu[i], 0.0, newSeeds[i], ddt[i],0)/util[i];
				}
				else{
					//state.output.systemMessage("seeds: "+seeds[i]);
					value = value + simulate(state, ind, subpopulation, threadnum, util[i], DIST.UNIFORM, mu[i], 0.0, seeds[i], ddt[i],0)/util[i];
				}
			}
			...
			((HHFitness)ind.fitness).setFitness(state,(float)(value/((double)(seeds.length))),false);
			if(VERB==1)  state.output.systemMessage("Fitness: "+(value/((double)(seeds.length))));
  ...
  /* throughout the simulation of the job shop.*/
			...
			int test)
	{
		...
		Qsize = size;
		createJobs(state,ind,subpopulation,threadnum,util,dist,mu,param2,seed,ddt,test);
		...
		while(collected < nmeas){
			...
			if(njobs < jobs.length){
				...
				return 10000000; // rule is bad at scheduling and tonnes of jobs have arrived before the nmeas jobs after warmup were processed.
			}
			...
			if(nextMachReady < nextArrival)
			{
				...
				if(currentMachine.getNumJobQueue()>0){
					...
					evaluateQueue(state, ind, subpopulation, threadnum);
				}
			}
			else if(njobs < size)
			...
			final int threadnum)
	{
		...
		if(VERB==1)  ind.printIndividualForHumans(state, 1);
		...
		for(j=0; j < currentMachine.getNumJobQueue(); j++){
			...
			// TODO modify this part to fit the approach that I'm using.
			if((numMachEvol<numMachines)){
				if(currentMachine.getMachineID()<numMachEvol){
					...
					currentJob.setPriority(rd.x);
				}
				else{
					//currentJob.setPriority(j); //FIFO
					//currentJob.setPriority(-1*currentJob.getDuedate()); // EDD
					currentJob.setPriority(-1*((double)(currentJob.getPR()))/currentJob.getWeight()); // WSPT
				}
			}
			else{
				((GPIndividual)ind).trees[0].child.eval(state,threadnum,rd,stack,((GPIndividual)ind),this);
			...
			if(j==0) max = currentJob.getPriority();
			...
			if(Math.abs(max-currentJob.getPriority())<0.0001){
