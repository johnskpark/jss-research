...
import ec.util.*;
import ec.*;
import ec.app.TwoDStripPacking.BestFit.BestFitEfficientPenalty;
import ec.gp.*;
import ec.gp.koza.*;
import ec.simple.*;


public class GPtwoDstripPenalty extends GPProblem implements SimpleProblemForm {
    int[] index =  {10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,13,3,3,3,3,3,3,3,6,6
    ...
    BestFitEfficientPenalty[][] TrainingInstances;
    int[] TrainInstanceSet = {3};

    public Object clone(){
        GPtwoDstripPenalty newobj = (GPtwoDstripPenalty) (super.clone());
    ...
    public void setup(final EvolutionState state,final Parameter base) {
        ...
        TrainingInstances = new BestFitEfficientPenalty[TrainInstanceSet.length][];
            for (int i = 0; i < TrainInstanceSet.length; i++) {
                TrainingInstances[i] = new BestFitEfficientPenalty[index[i]];
                for (int j = 0; j < index[TrainInstanceSet[i]]; j++) {
                     TrainingInstances[i][j] = new BestFitEfficientPenalty(TrainInstanceSet[i], j);
                }
            }
        ...
        if (!ind.evaluated)  // don't bother reevaluating
            ...
            for (int i = 0; i < TrainInstanceSet.length; i++) {
                for (int j = 0; j < index[TrainInstanceSet[i]]; j++) {
                     ...
                     //result.add(0.000001+(obj-200)/200);
                }
            }
        ...
        private String getTestPerformance(final EvolutionState state, final int threadnum, Individual best_i, int startIndex, int nInstances){
            ...
            for (int i = 0; i < TestInstanceSet.length; i++) {
                ...
                for (int j = 0; j < index[i]; j++) {
                     BestFitEfficientPenalty problem = new BestFitEfficientPenalty(TestInstanceSet[i], j);
