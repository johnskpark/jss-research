...
import ec.util.*;
import ec.*;
import ec.app.TwoDStripPacking.BestFit.BestFitEfficient;
import ec.gp.*;
import ec.gp.koza.*;
import ec.simple.*;


public class GPtwoDstrip extends GPProblem implements SimpleProblemForm {
    ...
    BestFitEfficient[][] TrainingInstances;
    int[] TrainInstanceSet = {76};

    public Object clone(){
        GPtwoDstrip newobj = (GPtwoDstrip) (super.clone());
    ...
    public void setup(final EvolutionState state,final Parameter base) {
        ...
        TrainingInstances = new BestFitEfficient[TrainInstanceSet.length][];
            for (int i = 0; i < TrainInstanceSet.length; i++) {
                TrainingInstances[i] = new BestFitEfficient[index[i]];
                for (int j = 0; j < index[TrainInstanceSet[i]]; j++) {
                     TrainingInstances[i][j] = new BestFitEfficient(TrainInstanceSet[i], j);
                }
            }
        ...
        private String getTestPerformance(final EvolutionState state, final int threadnum, Individual best_i, int startIndex, int nInstances){
            ...
            for (int i = 0; i < TestInstanceSet.length; i++) {
                ...
                for (int j = 0; j < index[i]; j++) {
                     BestFitEfficient problem = new BestFitEfficient(TestInstanceSet[i], j);
