...
public class QCSPmo {
    ...
    public boolean unidirection = false;
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length!=0){
            ...
            QCSPmo.uncertainty = Boolean.parseBoolean(args[5]);
            QCSPmo.noiseLevel = Double.parseDouble(args[6]);
            outname = args[7];
        }
        QCSPmo[][] test = new QCSPmo[nSet][nInPerSet];
        ...
        for (int i = 0; i < nSet; i++) {
            for (int j = 0; j < test[i].length; j++) {
                test[i][j] = new QCSPmo(QCSPmo.dataset[i+fromT] + df2.format(j + fromInstanceT + 1) + ".txt");
        ...
        for (int i = 0; i < nSet; i++) {
            for (int j = 0; j < nInPerSet; j++) {
                ...
                for (int k = 0; k < nRep; k++) {
                    ...
                    avgREL[i][j].add((OBJ[i][k][j]-QCSPmo.bestKnown[i+fromT][j+fromInstanceT])/QCSPmo.bestKnown[i+fromT][j+fromInstanceT]);
        ...
        writeResult("nqcspREPORT" + QCSPmo.uncertainty + outname, report + "\n === robust test === \n" + testReport, false);
        System.out.println("\n === robust test === \n" + testReport);
    ...
    public int[] getOrderInt(ArrayList<Integer> seq){
        int[] order = new int[seq.size()];
        int count = seq.size();
        for (int i : seq) {
            order[i] = count-1;
            count--;
    ...
    public QCSPmo(String problemfile) throws IOException{
    ...
    int rep;
    double bestECmax;
    public SmallStatistics stat = new SmallStatistics();
    public double constructSchedule(){
        stat = new SmallStatistics();
        double e_cmax = 0;
        double cmax = 0;
        double[] pen = new double[n];
        ...
        if (uncertainty){
            ...
            for (int i = 0; i < repTrain; i++) {
                System.arraycopy(pen, 0, penalty, 0, n);
                double val = constructSchedule(i);
                stat.add(val);
                e_cmax += val;
            }
        }
        else if (LOCAL_MODE) {
            double val = constructSchedule(-1);
            stat.add(val);
            e_cmax += val;
        }
        else {
...
//---------------------------------------------------------
    public double[] obj = new double[2];
    public double[] refobj = new double[2];
    public double constructSchedule(int seed){
        ...
        while(!tasks.isEmpty()){
            ...
            for (int task:tasks){
                if (!LOCAL_MODE){
                    ...
                    double W = w[task];
                    data.updateData(W,B, Q, PR, HWL, D, LWL, LQC, DNQ, C, CNQ, NBQT, T, S, RFT, PT);
        ...
        if (ub>cmax) ub = cmax;
        obj[0] = cmax;
        obj[1] = avgWeightCompleteTime();
        refobj[0] = (cmax);
        refobj[1] = avgWeightCompleteTime();
        //obj[2] = avgQuayCompleteTime();
        return cmax;
    }
...
////////////////////////////////////
    ...
    public double avgQuayCompleteTime(){
        double total = 0;
        for (int i = 0; i < q; i++) {
            total += c[i];
        }
        return total/(double)q;
    }
    public double avgWeightCompleteTime(){
        double total = 0;
        for (int i = 0; i < n; i++) {
            total += w[i]*ft[i];
        }
    ...
    public double[] w;
    public double lbwcmax = 0;
    public double getLBWCmax(){ //using the optimal rule for single machine WSPT
        for (int i = 0; i < n; i++) {
            penalty[i] = w[i]/p[i];
        }
        LOCAL_MODE = true;
    ...
    public void convertToPenalty(int[] sol){
