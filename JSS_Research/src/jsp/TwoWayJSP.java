...
public class TwoWayJSP {
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException, IllegalArgumentException, MathException {
        ...
        for (int i = 0; i < 0; i++) {
            ...
            x.clear(); y.clear();
            double[][][][][][] e1 = Dynamic2Way_SP.simDDAM("DTWK",true,true,1,100000,320000,x,y,coff);
            //double[] tempCoff = coff.clone(); 
            coff = doRegression(x, y);
            //if (calDiffPercentage(tempCoff,coff)<=0.01) break;
            writeArray(coff);
            if (tempMeasure > mean(e1[0][0][0][0][5])) {
                tempMeasure = mean(e1[0][0][0][0][5]);
        ...
        ArrayList x = new ArrayList();
        ArrayList y = new ArrayList();        
        double[][][][][][] e1 = Dynamic2Way_SP.simDDAM("ADRES",true,true,1,1000,5000,x,y,coff);
        System.out.println("++++++++++++++++++++++++++++++++++++++++++++++++");
        System.out.print("{");
        for (int i = 0; i < 0; i++) {
            for (int o = 0; o < 3; o++) {
                System.out.print(e1[0][0][0][0][o][i] + ","); 
            } 
            System.out.println("");
        }
        System.out.println("};");
