...
public class IterativeMainVNSLookahead {
    ...
    for (double k = 4.40; k < 4.5; k+=0.5) {
        ...
        for (int ds = 0; ds < 2; ds++) {
            ...
            for (int i = 0; i < jspTesting.length; i++) {
                ...
                //jspTesting[i].getJSPdata(i*2*0 + 1*95+1*i +0*ds+ 1);
           ...
           //DMU - instances (1-80)//la - instances (81-120)
            ...
            for (int i = 0; i < jspTesting.length; i++) {
                ...
                boolean[] isApplied_Nk = new boolean[5]; //Arrays.fill(isApplied_Nk, Boolean.TRUE);
                ...
                //choose the next machine to be schedule
                    while (nScheduledOp<N){
                        ...
                        jspTesting[i].setNonDelayFactor(0.4);
                        //jspTesting[i].setNonDelayFactor(0.4);
                        //*
                        ...
                        for (Job J:M.getQueue()) {
                            ...
                            if (Nk==0)
                                J.addPriority(RJ - ((PR * RFT) / W));
                            else if (Nk==1)
                                J.addPriority((-1*((PR * RFT) / ((W / RT) / (RFT - W))) + ((min(max(RO,RT),((W / PR) + min(0.026903434,PR))) / (RT + RFT)) + min(0.026903434,PR))) + IF(0.44284365 + W, RFT + RFT, max(PR,DD)));
                            else if (Nk==2)
                                J.addPriority(min(max(RM,RWT),((W / PR) + min(0.026903434,PR))) / IF(0.44284365 + W, Math.abs(RFT - RO) / 0.19508022, Math.abs(RT - RFT) / 0.19508022));
                            else if (Nk==3)
                                J.addPriority(min(max(RO,RT),((W / PR) + min(max(RM,RWT),((W / PR) + min(0.026903434,PR))))) / max(IF(0.44284365 + W, Math.abs(RT - RFT) / 0.19508022, (PR * RFT) + (RT - RFT)),(-1*(0.024657072) - min(DD,W))));
                            else if (Nk==4)
                                J.addPriority((min(max(RO,RT),((W / PR) + min(0.026903434,PR))) / max(IF(0.44284365 + W, Math.abs(RT - RFT) / 0.19508022, PR * RFT),(max(PR,DD) - min(DD,W)))) / max(W,(RFT + RFT)));
                        }
                        //*
                        if (Nk==0) jspTesting[i].setPriorityType(Machine.priorityType.W_CR_SPT);
                        else if (Nk==1) {
                            Machine.b = 0.5; Machine.k = 1.5;
                            jspTesting[i].setPriorityType(Machine.priorityType.ATC);
                        }
                        else if (Nk==2) {
                            Machine.b = 2.5; Machine.k = 2.5; 
                            jspTesting[i].setPriorityType(Machine.priorityType.CONV);
                        } else if (Nk==3) jspTesting[i].setPriorityType(Machine.priorityType.WSPT);
                        else if (Nk==4) jspTesting[i].setPriorityType(Machine.priorityType.W_SRPT_SPT);
                    ...
                    if (tempObj > currentObj){
                        ...
                        if (!isNextApplied(Nk, isApplied_Nk)) Nk = circleShift(Nk, isApplied_Nk.length);
                            ...
                            } else jspTesting[i].restoreBestRecordSchedule();
                            if (countLargeStep<1) {
                ...
                //System.out.println(jspTesting[i].instanceName + " & "+ globalBest + " & " + jspTesting[i].Machines.length*jspTesting[i].Jobs.length + " & " + countEval);
                //System.out.print(jspTesting[i].Machines.length*jspTesting[i].Jobs.length + "," + countEval);
                Size +=jspTesting[i].Machines.length*jspTesting[i].Jobs.length+",";
                Steps += countEval +",";
                Quality += jspTesting[i].getDevREFTotalWeightedTardiness(globalBest) +",";
            }
        }
