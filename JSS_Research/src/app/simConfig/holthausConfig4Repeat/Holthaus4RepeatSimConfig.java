...
public class Holthaus4RepeatSimConfig extends DynamicBreakdownSimConfig {
	...
	public Holthaus4RepeatSimConfig(List<Double> repairTimeFactors,
		...
		numConfigs = numScenarios * numDDFs * numRepeat;
	...
	public DblStream getDueDateFactor(int index) {
		...
		int ddfIndex = repeatIndex % ddfs.size();
	...
	public double getBreakdownLevel(int index) {
		...
		int blIndex;
		if (hasZeroBL) {
			if (repeatIndex < ddfs.size()) {
				blIndex = 0;
			} else {
				blIndex = (repeatIndex - ddfs.size()) / (rtfs.size() * ddfs.size()) + 1;
			}
		} else {
			blIndex = repeatIndex / (rtfs.size() * ddfs.size());
		}
	...
	public double getMeanRepairTime(int index) {
		...
		if (getBreakdownLevel(repeatIndex) == 0.0) {
			return 1.0;
		}
		...
		if (hasZeroBL) {
			if (repeatIndex < ddfs.size()) {
				...
				rtfIndex = ((repeatIndex - ddfs.size()) / ddfs.size()) % rtfs.size();
			}
		} else {
			rtfIndex = (repeatIndex / ddfs.size()) % rtfs.size();
		}
