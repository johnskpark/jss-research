...
public class CoopSPEA2Evaluator extends MultiPopCoevolutionaryEvaluator {

	private static final long serialVersionUID = -8595313908846862183L;
	...
	public void computeAuxiliaryData(EvolutionState state, Individual[] inds) {
		...
		for(int y = 0; y < inds.length; y++) {
			...
			SPEA2MultiObjectiveFitness fitness = (SPEA2MultiObjectiveFitness) inds[y].fitness;
			int myStrength = 0;
			for(int z = 0; z < inds.length; z++) {
				if (fitness.paretoDominates((MultiObjectiveFitness) inds[z].fitness)) {
					myStrength++;
				}
			}
			fitness.strength = myStrength;
		...
		for(int y = 0; y < inds.length; y++) {
			...
			for(int z = 0; z < inds.length; z++) {
				// Raw fitness
				SPEA2MultiObjectiveFitness rawFitness = (SPEA2MultiObjectiveFitness) inds[z].fitness;
				if (rawFitness.paretoDominates((MultiObjectiveFitness) inds[y].fitness)) {
					fitness += rawFitness.strength;
				}
	...
	public double[][] calculateDistances(EvolutionState state, Individual[] inds)
	{
		...
		for(int y = 0; y < inds.length; y++) {
			...
			for(int z = y + 1; z < inds.length; z++) {
				distances[z][y] = distances[y][z] = fitness.sumSquaredObjectiveDistance((SPEA2MultiObjectiveFitness) inds[z].fitness);
			}
		}
		return distances;
	}
