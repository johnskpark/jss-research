...
public class DynamicCostBasedTest {
    /**
    ...
    */
    public static void main(String[] arg) throws FileNotFoundException, IOException{
        String[] rules = {"CR_SPT","CR","EDD","FDD","FIFO","LIFO","LPT","AVPRO","LWKR","LWKR_SPT","MOD","MOPNR","MWKR","NPT","OPFSLK_PT","PW","SL","Slack","Slack_OPN","Slack_RPT_SPT","SPT_PW","SPT_PW_FDD","PT_WINQ","2PT_WINQ_NPT","PT_WINQ_SL","PT_WINQ_NPT_WSL","ATC","COVERT","RR","SPT","WINQ"};
        DynamicCostBasedTest.simDDAM(rules,true, true, 50, 500, 2000);
    }
    public static double[][][][][][][] simDDAM(String[] rules, boolean writeJobInfo, boolean writeReport, int rep, int warm, int end) throws FileNotFoundException, IOException {
            ...
            for (String dist : dists){for (String s : lowers){ for (int m : numbeOfMachines){ for (String rule : rules){ for (double mean : Mean){  for (double u : utilisation){ for (double al: allowance){
                if (rule!="WINQ") continue;
...
//duniform miss 10 0.85         665.7556726604303 2334.885706786335 0.349029054660297 56.72837705046707 737.249260098178
                                        ...
                                        else if("EVO".equals(rule)) J.addPriority(PR * (max(WINQ , PR) * (PR * (-1*(EFT) - IF(SJ, rJ - RT, RO)))));
                                        else
