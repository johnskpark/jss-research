...
package TwoWaySchedulingPolicy;
...
public class Dynamic2Way_SP {
    ...
    */
    public static double[][][][][][] simDDAM(String rule, boolean writeJobInfo, boolean writeReport, int rep, int warm, int end, ArrayList x, ArrayList y, double[] coff) throws FileNotFoundException, IOException {
            ...
            for (String dist : dists){for (String s : lowers){ for (int m : numbeOfMachines){ for (double u : utilisation){
                ...
                for (int ds = 0; ds < rep; ds++) {
                    ...
                    if (jspDynamic.isNextArrivalEvent()) {
                        ...
                        //newjob.assignDuedate(coff[0]*TOT + coff[1]*JQ + coff[2]*TLOT + coff[3]*TQWL + coff[4]*NO); newjob.extraInfo= TOT + " " + JQ + " " + TLOT + " " + TQWL + " " + NO;
                                                //DTWK
                        double flow =0;
                        if (rule.equals("DTWK")) flow = max(1,N/(lamda*up*ug))*TOT;
                        //DPPW
                        if (rule.equals("DPPW")) flow = TOT + Nq*NO/(lamda*ug);
                        //ADRES
                        if (rule.equals("ADRES")) flow = TAPR + jspDynamic.getTotalADRESWaitingTime(newjob);
                        //newjob.assignDuedate(flow);
                        jspDynamic.setNextArrivalTime();
                        } else {
                            ...
                            do {
                                ...
                                if (M.getQueue().size()>1){
                                    for (Job J:M.getQueue()) {
                                        ...
                                        double temp = 2*RO*RO*MP/(0.9087407-RJ)+IF(max(BWR,WR)-2*SJ,-PR,-RJ);
                                        J.addPriority(temp);
                                    }
                ...
                if (writeReport){
                    ...
                    detailedReport+=setting + Cmax.getAverage()+ " " + TWT.getAverage()+ " " + MPEA.getAverage() 
                            + " " + MPE.getAverage() + " " +
                ...
                System.out.println("Performance on test set: [Cmax = " + result[0].getAverage() +
                        ", TWT = " + result[1].getAverage() + ", MAPE = " +
                        result[2].getAverage() + detailedReport);
                System.out.println("{" + result[0].getAverage() + "," + result[1].getAverage() + "," + result[2].getAverage()+ "}") ;
                return detailedTest;
    }
